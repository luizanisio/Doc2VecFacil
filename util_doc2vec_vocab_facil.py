...
def criar_arquivo_curadoria_termos(pasta_textos, arquivo_saida, pasta_vocab = None, gerar_curadoria = True):
    arquivo_saida = arquivo_saida if arquivo_saida else 'curadoria_vocab.txt'
    arquivo_saida_oov = None
    arquivo_saida_oov_quebrados = None
    arquivo_saida_ok_quebrados = None
    arquivo_saida_vocab = None
    if arquivo_saida.lower().endswith('.txt'):
        _arq_saida = str(arquivo_saida).lower()[:-4]
        arquivo_saida_oov = _arq_saida + ' (OOV).txt'
        arquivo_saida_oov_quebrados = _arq_saida + ' (OOV QUEBRADOS).txt'
        arquivo_saida_oov_quebrados_tokens = _arq_saida + ' (OOV QUEBRADOS - TOKENS).txt'
        arquivo_saida_ok_quebrados = _arq_saida + ' (OK QUEBRADOS).txt'
        arquivo_saida_vocab = _arq_saida + ' (VOCAB FINAL).txt'
    ####################################################################################################
    print('=============================================================')
    ...
    print('\t - compilando dicionário de termos')
    ...
    arq_oov_pesos = f'{arquivo_saida[-3:]} (OOV pesos).txt' if arquivo_saida.lower().endswith('.txt') else f'{arquivo_saida} (OOV pesos).txt'
    with open(arq_oov_pesos,'w') as fo:
...
if __name__ == "__main__":
    ...
    ARQUIVO_CURADORIA = os.path.join(PASTA_MODELO,'curadoria_vocab.txt')
    ...
    ARQUIVO_VOCAB_CURADORIA = os.path.join(PASTA_MODELO,'curadoria_vocab.txt')
    ...
    if  (not os.path.isfile(ARQUIVO_VOCAB_AUTOMATICO)):
        ...
        tokenizador = criar_arquivo_curadoria_termos(pasta_textos = PASTA_TEXTOS, 
                                                     arquivo_saida=str(ARQUIVO_VOCAB_CURADORIA), 
    ...
    criar_arquivo_curadoria_termos(pasta_textos = pastas, arquivo_saida=str(ARQUIVO_VOCAB_CURADORIA), pasta_vocab=PASTA_MODELO)
    print('#---------------------------------------------------------#')
