...
class TokenizadorInteligente():
    RE_TOKENIZAR = re.compile('[^a-z0-9á-ú]')
    ...
    def __init__(self, pasta_vocab, registrar_oov = False, tokenizar_tudo = False):
        ...
        self.vocab_removido_composto = set()    # termos compostos removidos antes e depois do processamento
        if registrar_oov and self.pasta_vocab:
            self.nome_oov = os.path.join(self.pasta_vocab,'vocab_fora.txt')
            self.nome_oov_quebrados = os.path.join(self.pasta_vocab,'vocab_fora_quebrados.txt')
            self.nome_oov_quebrados_tokens = os.path.join(self.pasta_vocab,'vocab_fora_quebrados_tokens.txt')
            self.nome_ok_quebrados = os.path.join(self.pasta_vocab,'vocab_ok_quebrados.txt')
            self.nome_vocab_final = os.path.join(self.pasta_vocab,'vocab_final.txt')
        ...
        if self.pasta_vocab:
            ...
            print(f'\t Pasta do vocab não definida, todos os termos serão tokenizados termos', flush=True)
    ...
    def carregar_vocabs(self):
            ...
            for path, dir_list, file_list in os.walk(self.pasta_vocab):
                for file_name in file_list:
                    if file_name.lower().endswith(".txt"):
                        ...
                        if file_name.lower()[:23]=='vocab_removido_composto':
                            pathfile_name = os.path.join(path,file_name)        
                            _removido_composto +=  carregar_arquivo(pathfile_name,limpar=False,juntar_linhas=False)
                            print(f'\t - vocab removido composto: {file_name} carregado o/')
        ...
        # vocab removido = termos compostos 
        # - ordena do maior para o menor para excluir primeiro os termos maiores que podem conter outros menores
        self.vocab_removido_composto = []
        for composto in _removido_composto:
            composto = self.remover_acentos(composto.lower().replace('<br>',' ').replace('\n',' '))
            composto = self.REGEX_SIGLAS.sub('', ' ' + composto + ' ').strip()
            composto = composto.strip()
            if composto and composto not in self.vocab_removido_composto:
                self.vocab_removido_composto.append(composto)
        self.vocab_removido_composto = [ _ for _ in sorted(self.vocab_removido_composto, key = lambda k:len(k), reverse=True) ]
        #print('Termos compostos para remover')
        #print(list(self.vocab_removido_composto))
        ...
        txt_numeros = ' zero um dois tres quatro cinco seis sete oito nove '
        self.numeros = [_ for _ in txt_numeros.split(' ') if _]
        vocab += txt_numeros
    ...
    def tokenizar(self, sentenca):
        sentenca = self.remover_acentos(sentenca.lower().replace('<br>',' ').replace('\n',' '))
        sentenca = self.REGEX_SIGLAS.sub('', ' ' + sentenca + ' ')

        # remove os termos compostos se existirem
        for composto in self.vocab_removido_composto:
            sentenca = sentenca.replace(composto,' ')
        ...
        tokens = self.RE_TOKENIZAR.sub(' ',sentenca).split(' ')
...
if __name__ == "__main__":
    ...
    parser.add_argument('-dimensoes', help='número de dimensões para treinamento - padrão 200 ', required=False, action='store_const', const=200)
    parser.add_argument('-workers', help='threads de treinamento - padrão 100 ', required=False, action='store_const', const=100)
    ...
    doc2vecTreina = UtilDoc2VecFacil_Treinamento(pasta_modelo= PASTA_MODELO,
                                                    ...
                                                    epocas=args.epocas or 5000,
                                                    dimensoes=args.dimensoes or 200,
                                                    min_count=5,
                                                    workers=args.workers or 100,
