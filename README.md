...
## Como funciona o Tokenizador Inteligente <a name="tokenizador">
  ...
  - Você pode criar listas de termos que serão excluídos do treinamento, basta estaram em arquivos com o padrão `VOCAB_REMOVIDO*.txt` - aqui geralmente ficam 'stopwords' ou termos que não ajudam na diferenciação dos documentos. Os termos na lista de termos removidos não são fragmentados.
